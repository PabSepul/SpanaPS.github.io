...
for (const id of ['python', 'html-css', 'javascript', 'sql']) {
  const offset = id === 'python' ? 1 : 0;
...
assert.match(root.querySelector('#continue-description').textContent, /Proyecto 5 de 12/);
...
assert.equal(pythonCard.querySelector('[data-route-progress]').textContent, '4/12 proyectos');
assert.match(pythonCard.querySelector('[data-route-detail]').textContent, /1\/3 exámenes/);
assert.equal(pythonCard.querySelector('[data-route-fill]').style.width, '33%');
home.storage.set('codigo-cero.python-v2.completed', JSON.stringify(Array.from({length:12},(_,i)=>i+1)));
home.storage.set('codigo-cero.python-v2.exams','[1,2,3]');
events.storage();
